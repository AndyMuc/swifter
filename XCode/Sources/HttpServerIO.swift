...
open class HttpServerIO {
    ...
    public func start(_ port: in_port_t = 8080, forceIPv4: Bool = false, priority: DispatchQoS.QoSClass = DispatchQoS.QoSClass.background, writeChunkSize: Int? = nil,
                      writePeriodicDelay: TimeInterval? = nil) throws {
        guard !self.operating else { return }
        stop()
        self.state = .starting
        let address = forceIPv4 ? listenAddressIPv4 : listenAddressIPv6
      self.socket = try Socket.tcpSocketForListen(port, forceIPv4, SOMAXCONN, address, writeChunkSize: writeChunkSize, writePeriodicDelay: writePeriodicDelay)
        self.state = .running
